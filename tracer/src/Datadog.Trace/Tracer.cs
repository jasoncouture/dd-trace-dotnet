...
namespace Datadog.Trace
{
    ...
    public class Tracer : ITracer, IDatadogTracer, IDatadogOpenTracingTracer
    {
        ...
        internal Span StartSpan(string operationName, ITags tags, ISpanContext parent = null, string serviceName = null, DateTimeOffset? startTime = null, bool ignoreActiveScope = false, ulong? traceId = null, ulong? spanId = null, bool addToTraceContext = true)
        {
            ...
            if (parent != null)
            {
                // always use parent if specified
                span = new Span(traceContext, parent, spanId, startTime, tags);
            }
            else
            {
                ISpanContext parentSpanContext = null;

                if (!ignoreActiveScope)
                {
                    // get parent span id from active scope
                    parentSpanContext = DistributedTracer.Instance.GetSpanContext() ?? (ISpanContext)TracerManager.ScopeManager.Active?.Span;
                }

                span = new Span(traceContext, parentSpanContext?.SpanId, spanId, startTime, tags);
            }

            span.OperationName = operationName;
            span.ServiceName = serviceName ?? DefaultServiceName;
